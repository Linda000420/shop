    ...
    <div class="info">
      <div class="title">
        <div class="price">
          ...
          <span class="oldprice">￥{{ list.goods_price_max }}</span>
        </div>
...
<script>
import { getGoodsDetailList, getGoodsComments } from '@/api/goods'
...
export default {
  ...
  methods: {
    ...
    async getList () {
      const res = await getGoodsDetailList(this.goodsId)
      this.list = res.data.detail
      this.images = this.list.goods_images
    },
    async getComments () {
    ...
    async addCart () {
      ...
      const { data } = await addCart(this.goodsId, this.addCount, this.list.skuList[0].goods_sku_id)
