<template>
  <div class="order-list-item">
    ...
    <div class="list">
      <div class="list-item" v-for="item in item.goods" :key="item.goods_id">
        <div class="goods-img">
    ...
    <div class="total">
      共{{ item.total_num }}件商品，总金额 ¥{{ item.order_price }}
    </div>
    <div class="actions">
      <div v-if="item.order_status === 10">
        <span v-if="item.pay_status === 10">立刻付款</span>
        <span v-if="item.delivery_status === 10">申请取消</span>
        <span v-if="item.delivery_status === 20 || item.delivery_status === 30">确认收货</span>
      </div>
